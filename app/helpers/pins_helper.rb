module PinsHelper
  ...
  def render_pin_thumbnail(pin, width, height)
    if pin.image_url.present?
      link_to image_tag(pin.image_url, style: 'width: 100%; height: auto;'), pin, data: { turbo_frame: "_top" }
    elsif pin.image.attached?
      link_to image_tag(pin.resize_img(pin.image, width, height)), pin, data: { turbo_frame: "_top" }
    end
  end
